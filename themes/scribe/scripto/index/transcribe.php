...
jQuery(document).ready(function() {
    ...
    jQuery('#scripto-page-show').click(function(event) {
	    ...
	    jQuery('#discussion-tips').show(); //add
            jQuery('#scripto-page-show').text('<?php echo __('show transcription'); ?>');
        } else {
            window.location.hash = '#transcription';
            jQuery('#scripto-talk').hide();
	    ...
	    jQuery('#transcription-tips').show(); //add
            jQuery('#scripto-page-show').text('<?php echo __('show discussion'); ?>');
        }
    });
...
<?php echo flash(); ?>
 ...
 <!--           <li><span><?php echo __('Logged in as %s', '<a href="' . html_escape(url('scripto')) . '">' . $this->scripto->getUserName() . '</a>'); ?></span></li>
	...
	<!--commented out import document link -->
	...
	<!-- REMOVED H3 LINK -->
       ...
       <!-- <h3><//?php echo $this->doc->getPageName(); ?></h3>-->
        ...
        <div class="doc-column"><?php echo file_markup($this->file, array('imageSize' => 'fullsize')); ?>
        <!-- this is maddy's attempt to create a quick tips. note: it is within .doc-column and uses jquery .hide and .show-->
                <div id="transcription-tips">
                     <h2 style="padding-left:10px" ><u>Transcription Instructions</u></h2>
                    <p style="margin-bottom:0"><b> DO:</b></p>
                    <ul style="margin-top:0">
                    <li>Type what you see in the pdf, even if it's misspelled or incorrect.</li>
                    <li>Type page numbers if they appear.</li>
                    <li>Put unclear words in brackets, with a question mark, like: [Haines?]</li>
                    <li>Click "Save changes" frequently!</li>
                    </ul>
                    <p style="margin-bottom:0"><b>DON'T:</b></p>
                    <ul style="margin-top:0">
                    <li>Include hyphens splitting words at the end of a line. Type the full word without the hyphen. If a hyphen appears at the end of a page, type the full word on the second page.</li>
                    </ul>
                        <center> <p>For more instruction click<b> <a href="http://ds.haverford.edu/tichadocs_scribe/transcription_tips">here</a></b></p></center>
                </div>
                <div id="discussion-tips">
                     <h2 style="padding-left:10px"><u>Discussion Instructions</u></h2>
                    <p style="margin-bottom:0"><b> DO:</b></p>
                    <ul style="margin-top:0">
                    <li>To create title for your comment, type ==[your title]== and enter your comment on the next line </li>
                    <li>If you want to comment on a previous comment, leave the line underneath the comment blank and on the next like add two space and enter your comment.</li>
                    <li>Sign your comment with four tildes or "~~~~", this will add your user name and timestamp</li>
                    </ul>
                    <p style="margin-bottom:0"><b>DON'T:</b></p>
                    <ul style="margin-top:0">
                    <li>Delete any user's comments.</li>
                    <li>Don't forget to sign your comment!</li>
                    </ul>
                </div>
        </div>
<!--end of maddy's attempt-->
        <!-- transcription -->
<div class="transcribe-column">
        ...
        <h2><?php echo __('Current Page Transcription'); ?>
         ...
         <div>
             ...
             <?php if ($this->scripto->canExport()): ?><?php echo $this->formButton('scripto-transcription-page-import', __('Import Finished Page'), array('style' => 'display:inline; float:none;')); ?><?php endif; ?>
         </div>
        ...
        <?php else: ?>
                ...
                <div><?php echo $this->formTextarea('scripto-transcription-page-wikitext', $this->doc->getTranscriptionPageWikitext(), array('cols' => '76', 'rows' => '16' )); ?>
                </div>
                <?php endif; ?>
                        <div>
                        <?php echo $this->formButton('scripto-transcription-page-edit', __('Save Changes'), array('style' => 'display:inline; float:none;')); ?>
                        </div>
        ...
        <?php else: ?>
                <p><?php echo __('You currently do not have permission to transcribe this page.'); ?></p>
        <p>Please<b> <a href="http://ds.haverford.edu/tichadocs_scribe/scripto/login">Login</a></b> to gain access.</p>
	<?php endif; ?>
     ...
     <!-- discussion -->
        <div id="scripto-talk">
                ...
                <?php if ($this->doc->canEditTalkPage()): ?> 
                        [<a href="#" id="scripto-talk-edit-show"><?php echo __('edit'); ?></a>]<?php endif; ?>
                <?php if ($this->scripto->canProtect()): ?>
                 [<a href="<?php echo html_escape($this->doc->getTalkPageMediawikiUrl()); ?>"><?php echo __('wiki'); ?></a>]
                <?php endif; ?>
            ...
            <?php else: ?>
            <p><?php echo __('You currently do not have permission to discuss this page.'); ?></p>
            <p>Please<b> <a href="http://ds.haverford.edu/tichadocs_scribe/scripto/login">Login</a></b> to gain access.</p>
	    <?php endif; ?>
